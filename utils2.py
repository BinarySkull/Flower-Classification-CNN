...
# This function builds the network based on the network that the user entered with rather simple fc layers.
# You will notice that the architecture here is a bit different than the one in the jupyter notebook,
# the reason for this is that I found this one to be more suitable for a variable architecture and yielded better results.
...
# this function is the biggest as it is the one which actually have the training loop in it.
def train_network(model, epochs, learning_rate, trainloader, validloader, gpu):
    ...
    # After studying the given notebooks this way was the one I liked the most
...
# This function evaluates our trained model based on our test data that the model didn't see before 
def evaluate_model(model, testloader, criterion, gpu):
    ...
    test_loss = 0
    test_accuracy = 0
    model.eval() # Convert model to evaluation mode to stop training and back probagation
    with torch.no_grad():
...
# This function saves the model in a checkpoint so we can leave it and come complete training after loading the old saved weights.    
...
# This function loads the weights.    
...
# This takes in a processed image and predicts its classification.
